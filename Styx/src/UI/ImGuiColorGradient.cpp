...
namespace Charon {
	...
	{
		auto iterator = std::find(m_marks.begin(), m_marks.end(), mark);
		if (iterator != m_marks.end())
		{
			m_marks.erase(iterator);
			refreshCache();
		}
	}

	void ImGradient::getColorAt(float position, glm::vec4& color) const
	{
		position = ImClamp(position, 0.0f, 1.0f);
	...
	void ImGradient::computeColorAt(float position, float* color) const
	{
		...
		if (position <= m_marks.front()->position)
		{
			upper = m_marks.front();
			color[0] = upper->color[0];
			color[1] = upper->color[1];
		...
		if (position >= m_marks.back()->position)
		{
			upper = m_marks.back();
			color[0] = upper->color[0];
			color[1] = upper->color[1];
	...
	{
		std::sort(m_marks.begin(), m_marks.end(), [](const ImGradientMark* a, const ImGradientMark* b) { return a->position < b->position; });
